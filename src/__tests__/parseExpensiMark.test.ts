...
const toBeParsedAs = function (actual: string, expectedRanges: MarkdownRange[]) {
  ...
  if (JSON.stringify(actualRanges) !== JSON.stringify(expectedRanges)) {
    return {
      pass: false,
      message: () =>
        `Expected ${JSON.stringify(expectedRanges)}, got ${JSON.stringify(actualRanges)}`,
    }
  }
...
describe("mention-user", () => {
  test("normal", () => {
    expect("@mail@mail.com Hello!").toBeParsedAs([
  ...
  test("with valid phone number", () => {
    expect("@+15005550006 Hello!").toBeParsedAs([
      { type: "mention-user", start: 0, length: 13 },
    ])
  })
})
...
describe("inline image", () => {
  ...
  test("text containing images", () => {
    expect(
      "An image of a banana: ![banana](https://example.com/banana.png) an image of a developer: ![dev](https://example.com/developer.png)",
    ).toBeParsedAs([
  ...
  test("with alt text containing markdown", () => {
    expect(
      "![# fake-heading *bold* _italic_ ~strike~ [:-)]](https://example.com/image.png)",
    ).toBeParsedAs([
  ...
  test("text containing image and autolink", () => {
    expect(
      "An image of a banana: ![banana](https://example.com/banana.png) an autolink: example.com",
    ).toBeParsedAs([
      { type: "inline-image", start: 22, length: 41 },
      { type: "syntax", start: 22, length: 1 },
      { type: "syntax", start: 23, length: 1 },
  ...
  test("trying to pass additional attributes", () => {
    expect('![test](https://example.com/image.png "title" class="image")').toBeParsedAs([
      { type: "link", start: 8, length: 29 },
    ])
  })
...
describe("report mentions", () => {
  ...
  test("report mention in tense", () => {
    expect("reported #report-name should be highlighted").toBeParsedAs([
      { type: "mention-report", start: 9, length: 12 },
    ])
  })
  ...
  test("report mention with punctuation", () => {
    expect("reported #report-name!").toBeParsedAs([
      { type: "mention-report", start: 9, length: 12 },
    ])
  })
})
...
describe("inline video", () => {
  ...
  test("with alt text containing markdown", () => {
    expect(
      "![# fake-heading *bold* _italic_ ~strike~ [:-)]](https://example.com/video.mp4)",
    ).toBeParsedAs([
      { type: "syntax", start: 0, length: 1 },
      { type: "syntax", start: 1, length: 1 },
      { type: "syntax", start: 47, length: 1 },
  ...
  test("trying to pass additional attributes", () => {
    expect('![test](https://example.com/video.mp4 "title" class="video")').toBeParsedAs([
      { type: "link", start: 8, length: 29 },
    ])
  })
