...
function parseHTMLToTokens(html: string): Token[] {
  ...
  while (true) {
    ...
    if (close === -1) {
      throw new Error(
        '[react-native-live-markdown] Error in function parseHTMLToTokens: Invalid HTML: no matching ">"',
      )
    }
...
function parseTokensToTree(tokens: Token[]): StackItem {
  ...
  tokens.forEach(([type, payload]) => {
    if (type === "TEXT") {
      ...
      throw new Error(
        `[react-native-live-markdown] Error in function parseTokensToTree: Unknown token type: ${
          type as string
        }. Expected 'TEXT' or 'HTML'. Please ensure tokens only contain these types.`,
      )
    }
  })
  if (stack.length !== 1) {
    ...
    if (typeof node === "string") {
      ...
      if (node.tag === "<>") {
        ...
        if (
          ...
          (node.children[0] === matchString || `mailto:${node.children[0]}` === href)
        ) {
        ...
        const start = text.length
        const length =
          3 + (hasAlt ? 2 + unescapeText(alt?.[1] || "").length : 0) + linkString.length
        ...
        throw new Error(
          `[react-native-live-markdown] Error in function parseTreeToTextAndRanges: Unknown tag '${node.tag}'. This tag is not supported in this function's logic.`,
        )
      }
    }
  }
