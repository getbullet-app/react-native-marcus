...
function getImagePreviewElement(targetElement: HTMLMarkdownElement) {
  return Array.from(targetElement?.childNodes || []).find(
    (el) => (el as HTMLElement)?.contentEditable === "false",
  ) as HTMLMarkdownElement | undefined
}
...
function handleOnLoad(
  ...
  if (!targetElement.isConnected) {
    const currentElement = currentInput.querySelector(
      `[data-type="block"][data-id="${target.getAttribute("data-id")}"]`,
    ) as HTMLMarkdownElement
  ...
  const currentSpinner = currentInput.querySelector(
    `[data-type="spinner"][data-url="${imageHref}"]`,
  )
...
function createImageElement(
  ...
  if (timeoutMap.has(targetNode.orderIndex)) {
    ...
    const currentElement = currentInput.querySelector(
      `[data-type="block"][data-id="${targetNode.orderIndex}"]`,
    ) as HTMLMarkdownElement
  ...
  const timeout = setTimeout(() => {
    ...
    img.onload = () =>
      handleOnLoad(currentInput, targetNode.element, url, markdownStyle, imageContainer)
    img.onerror = (err) =>
      handleOnLoad(currentInput, targetNode.element, url, markdownStyle, imageContainer, err)
    img.src = url
    timeoutMap.delete(targetNode.orderIndex)
...
function updateImageTreeNode(
  targetNode: TreeNode,
  newElement: HTMLMarkdownElement,
  imageMarginTop = 0,
) {
  const paddingBottom = getContainerPaddingBottom(
    parseStringWithUnitToNumber(newElement.style.height),
    imageMarginTop,
  )
  ...
  while (
    ...
    !["line", "block"].includes(currentParent.getAttribute("data-type") || "")
  ) {
...
function addInlineImagePreview(
  ...
  if (linkRange) {
    ...
    if (
      ...
      imagePreviewAuthRequiredURLs.find((url) => imageHref.startsWith(url))
    ) {
  ...
  const imageMarginTop = parseStringWithUnitToNumber(`${markdownStyle.inlineImage?.marginTop}`)
  const imageMarginBottom = parseStringWithUnitToNumber(
    `${markdownStyle.inlineImage?.marginBottom}`,
  )
  ...
  if (
    ...
    loadedImageContainer.getAttribute("data-type") === "inline-container"
  ) {
    return updateImageTreeNode(
      targetNode,
      loadedImageContainer as HTMLMarkdownElement,
      imageMarginTop,
    )
  }
  ...
  Object.assign(targetNode.element.style, {
    ...
    paddingBottom:
      markdownStyle.loadingIndicatorContainer?.height ||
      markdownStyle.loadingIndicator?.height ||
      (!!markdownStyle.loadingIndicator && "30px") ||
      undefined,
  })
...
function forceRefreshAllImages(
  ...
  currentInput?.querySelectorAll("img").forEach((img) => {
    ...
    imgElement.onload = () =>
      handleOnLoad(
        currentInput,
        img.parentElement?.parentElement as HTMLMarkdownElement,
        url,
        markdownStyle,
        img.parentElement as HTMLMarkdownElement,
      )
    imgElement.src = `${url}#`
  })
}
