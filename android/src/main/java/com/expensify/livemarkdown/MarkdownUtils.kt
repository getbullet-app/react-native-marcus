...
class MarkdownUtils(private val reactContext: ReactContext) {
  ...
  private val onSchemaError: (String) -> Unit = { error ->
    RNLog.w(reactContext, "[react-native-live-markdown] Incorrect schema of worklet parser output: $error")
  }
  ...
  fun applyMarkdownFormatting(ssb: SpannableStringBuilder) {
    try {
      ...
      val style = checkNotNull(markdownStyle) { "[react-native-live-markdown] markdownStyle is null" }
