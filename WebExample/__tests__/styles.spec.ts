...
test.describe("markdown content styling", () => {
  ...
  test("link", async ({ page }) => {
    await testMarkdownContentStyle({
      testContent: "https://expensify.com",
  ...
  test("mention-here", async ({ page }) => {
    await testMarkdownContentStyle({
      testContent: "here",
      style: "color: green; background-color: lime; border-radius: 5px;",
      page,
    })
  })
  ...
  test("mention-user", async ({ page }) => {
    await testMarkdownContentStyle({
      testContent: "someone@swmansion.com",
      style: "color: blue; background-color: cyan; border-radius: 5px;",
      page,
    })
  })
  ...
  test("mention-report", async ({ page }) => {
    await testMarkdownContentStyle({
      testContent: "mention-report",
      style: "color: red; background-color: pink; border-radius: 5px;",
      page,
    })
  })
  ...
  test("blockquote", async ({ page, browserName }) => {
    ...
    const browserStyle =
      browserName === "firefox"
        ? blockquoteStyle.replace("border-left-style: solid", "border-left: 6px solid gray")
        : blockquoteStyle
  ...
  test("blockquote with strikethrough", async ({ page, browserName }) => {
    ...
    await testMarkdownElementHasComputedStyle({
      testContent: "strikethrough_blockquote",
...
test.describe("empty input styling", () => {
  ...
  test("placeholder should have correct text", async ({ page }) => {
    const placeholder = await page.$eval(`div#${TEST_CONST.INPUT_ID}`, (el) =>
