...
const setCursorPosition = async (page: Page, elementIndex: number, offset?: number) => {
  return page.evaluate(
    ...
    async ({ elementIndex, offset }) => {
      const filteredNode = Array.from(
        document.querySelectorAll('span[data-type="text"], span[data-type="br"]'),
      )
...
const getElementValue = async (elementHandle: Locator) => {
  const inputValueHandle = await elementHandle.evaluateHandle(
    (div: HTMLInputElement) => div.value,
  )
...
const testMarkdownContentStyle = async ({
  testContent,
  style,
  page,
  dimmensions,
...
const testMarkdownElementHasComputedStyle = async ({
  testContent,
  propertyName,
  style,
  page,
  ...
  if (elementHandle) {
    await elementHandle.waitFor({ state: "attached" })
    elementStyle = await elementHandle.evaluate(
