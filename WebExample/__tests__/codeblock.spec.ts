...
async function testCodeblockStyle(
  page: Page,
  dimmensions: { height: number; width: number } | null,
  style: string | null = CODEBLOCK_DEFAULT_STYLE,
) {
  if (style === null) {
...
test.describe("modifying codeblock content", () => {
  ...
  test("remove newline before closing syntax with one empty line at the end", async ({
    page,
  }) => {
    const inputLocator = await setupInput(page, "clear")
    await inputLocator.focus()
  ...
  test("remove newline before closing syntax with two empy lines at the end", async ({
    page,
  }) => {
    const inputLocator = await setupInput(page, "clear")
    await inputLocator.focus()
