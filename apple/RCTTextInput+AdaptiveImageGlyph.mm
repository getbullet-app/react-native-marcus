...
// define
// `+load`.
static void RCTMarcusSwizzleAdaptiveImageGlyph(Class targetClass,
  ...
  if (@available(iOS 18.0, *)) {
    ...
    BOOL didAddMethod = class_addMethod(
        targetClass, originalSelector, method_getImplementation(swizzledMethod),
...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    RCTMarcusSwizzleAdaptiveImageGlyph([self class], @selector
                                       (marcus_insertAdaptiveImageGlyph:
                                                       replacementRange:));
  });
}
