...
// Installs `liveMarkdown_insertAdaptiveImageGlyph:replacementRange:` in place of
// UIKit's `insertAdaptiveImageGlyph:replacementRange:`. The handler itself is in
// Swift; only the runtime plumbing has to live here, because Swift cannot define
// `+load`.
static void RCTLiveMarkdownSwizzleAdaptiveImageGlyph(Class targetClass, SEL swizzledSelector)
{
...
+ (void)load
{
  ...
  dispatch_once(&onceToken, ^{
    RCTLiveMarkdownSwizzleAdaptiveImageGlyph(
        [self class], @selector(liveMarkdown_insertAdaptiveImageGlyph:replacementRange:));
  });
}
